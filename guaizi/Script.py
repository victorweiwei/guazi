...
jsFun = '''
    ...
    '''
jsContext = execjs.compile(jsFun)
...
def get_cookie(url):
    ...
    r = page.text
    r = re.search('var value=anti(.*?);var name=\'antipas\'', r)
    par = [r.group().split('\'')[1], r.group().split('\'')[3]]
    print(par)
    value = jsContext.call('anti', par[0], par[1])
    name = 'antipas'
    url = ''
    cookie = jsContext.call('xredirect', name, value, url, 'https://')
